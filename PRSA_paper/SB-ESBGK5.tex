...
Jaw-Yen Yang$^{1,2}$, Chin-Yuan Yan$^{1}$, Manuel Diaz$^{1}$, Juan-Chen Huang$^{3}$, Zhi-Hui Li$^{4}$}
...
\subject{Mesoscopic Methods, Semi-classical Botlzmann Transport, Ellipsoidal Statistics, Kinetic Theory, Quantum gas dynamics flow}
...
In principle, after solving the semiclasical Boltzmann-ES-BGK equation for the (non-equilibrium) distribution function $f$, all the macroscopic dynamic variables of interest such as number density, momentum density and energy density are low order moments of the distribution function and are defined by:
\begin{subequations}
\begin{align}
...
Specifically, we consider two configurations, namely the \emph{Configuration 5} and \emph{Configuration 17} among the 19 genuinely different configurations classified in \cite{Laxliu95}\cite{schultzrinne}. To satisfy these constraints, the set initial data must satisfy the Rankine-Hugoniot relations i.e., for given a pair of left and right states (denoted by the indices $l$ and $r$), we define the two configurations as follows.\\

\begin{figure}
	\centering
	\includegraphics[width=0.35\textwidth]{4quads}
  \caption{Domain configuration 2D Riemann problems.}
  \label{fig:quadrants}
\end{figure}
...
\end{align*}
The initial constant states at the four quadrants are specified as
...
\begin{figure}
        ...
        \end{subfigure}
				\caption{Results for \emph{Configuration 17} for three statistics using ES equilibrium distribution with parameter $b=0.5$ and TVD method.} 
...
For the \emph{Configuration 17}, again, the wave patterns shown in Fig. \ref{fig:test_configuration17} are consistent with calculations in \cite{Laxliu95}\cite{schultzrinne} for the same configuration and with same initial data.   Notable differences among the three statistics can be observed although the overall wave patterns are similar.  Quantitatively, the flow properties obtained by using Maxwell-Boltzmann statistics are always greater than those with obtained with Fermi-Dirac statistic and lower than those with Bose-Einstein statistics as it is expected.

\subsection{Mesh refiment tests}
Here we investigate consistency and convergence of our numerical implementation increasing their number in our domain and therefore reducing their size in our domain. Here we expect a decrese of the truncation error, and therefore observe converge paternt to the solution of the choosen configuration under test test. We report in Fig. \ref{fig:Mesh_refinement} our findinds in \emph{configurations 5} and \emph{configurations 17} using a grid $400\times400$, i.e. four-times the total of cells used in our initial test grid. We observe in Figs. \ref{fig:400x400_FD_ESBGK_n} and \ref{fig:400x400_FD_ESBGK_p} that slips structures exhibit much sharper profiles with a refined mesh. This also true in the vicinity of rarefaction fans and contact discontinuities by observing structures in Figs. \ref{fig:400x400_FD_ESBGK_n} and \ref{fig:400x400_FD_ESBGK_p}. Both results in Fig. \ref{fig:Mesh_refinement} are computed with \emph{CFl} condition equal to 0.6 and corresponds to an output time of $t=0.25$. Most of our results here presented are obtained using a grid $200\times200$ exhibitin a smeared profile compared to the ones grid $400\times400$, however these last ones required four-times more computational time to be completed.
...
\begin{figure}
        ...
        \end{subfigure}%
				\caption{Number density and pressure profiles for \emph{Configuration 5} for a Fermi ideal gas.}
				\label{fig:Mesh_refinement}
\end{figure}

\subsection{Computational study on dimensionles parameter $\it{b}$ for ES equilibrium distribution function}

Here, we investigate quatitatively and qualitatively the effects of dimensionless parameter $\it{b}$ in the ellipsoidal equilibrium probability distribution function model, $f^{ES}$, on the wave flow patterns and properties. Qualitative observations on different configurations, shown that parameter $\it{b}$ influences our results by increasing or decreseasing the sharpness of the profiles of every configurations. Quantitatively, we notice that it is not deterministic how this parameter it may affect the overall magitude of flow properites. We report this observations by using the number density contours for \emph{Configuration 5} evaluated with both quantum statistics in Fig. \ref{fig:test_b_parameter}. We choose for the discrete values of $b=\{-0.5,\;0,\;0.75\}$, number density wave patterns for Bose-Einstein and Fermi-Dirac with $b=0.5$ can be found in Figs, \ref{fig:5ESBGK_BE_n} and \ref{fig:5ESBGK_FD_n} respectively. Also notice that by setting $b=0$ we recover the traditional isotropic semiclassical distribution function, i.e., $f^{ES}= f^{eq}$. By observing figures \ref{fig:5ESBGK_BE_n}, one can find differences in the density number wave patterns and their numerical values however they still consisten with calculations in in \cite{Laxliu95}\cite{schultzrinne}.
...
\begin{figure}
        ...
        \end{subfigure}	
        \caption{A comparison on hydrodynamic manifestation of quantum gases in \emph{Configuration 5} and the influence of $\it{b}$ parameter in the Prandtl number of the configuration.}
...
In Figs. \ref{fig:conf5_W_p_Tensor} \& \ref{fig:conf5_W_p_Tensor2} the contours of $W_{\alpha \beta}$ and $P_{\alpha \beta}$ tensor components are depicted on \emph{Configuration 5} and ES model with $b=0.5$ using for ideal gases following Bose-Einstein and Fermi-Dirac statistic respectivelly. It is easy notice that contours of diagonal tensor components, $W_{xx}$ and $W_{yy}$, are very similar but a close look reveals that both differ slightly numerically. In contrast $W_{xy}$ component as depicted in figs. \ref{fig:5ESBGK_BE_bp05_Wxy} and \ref{fig:5ESBGK_FD_bp05_Wxy} are numerically small compared to the former ones. However, this component contribution cannot not be negleted in the present anisotropic model. \\
By looking at pressure tensor component in Figs. \ref{fig:conf5_W_p_Tensor} \& \ref{fig:conf5_W_p_Tensor2}, it is evident that $W_{\alpha \beta}$ and $P_{\alpha \beta}$ tensor quantities are closely related. This was expected by recalling relations (\ref{eq:pressure_tensor_variables}), moreover, it is also easy to notice that the contribution of main diagonal tensor components generally will be greater to the rest of the tensor components. \\
Comparing the pressure components, as before, we observe that the main daigonal components, $P_{xx}$ and $P_{yy}$, exhibit similar characteristic patterns but again slight numerical differences are identified. Notice that the main diagonal components will be equal whenever dimensionless paramter $\it{b = 0}$, i.e. $f^{eq}$ is used. Based on quantitative observations on several tested configurations, is easy to infer that is due to this slight differences triggered by the dimensionless parameter $\it{b}$ the ES distribution function creates on $P_{xx}$ \& $P_{yy}$, and the contribution of $P_{xy}$ components; that the anisotropic model contributes to relax the fixed Prandtl number condition of the traditional equilibrium distribution model, $f^{eq}$. \\
Notice the similitud between pressure contours in Figs. \ref{fig:conf5_W_p_Tensor} and \ref{fig:conf5_W_p_Tensor}, the results also show that numerical pressure values are greater for Bose-Einstein Statistics as expected. Finally, although the kind of gas used in this examples are Bose and Fermi gas, the results are comparable to the earlier work of Lax \cite{Laxliu95} (classical gas) in terms of the expected flow patterns.
...
\begin{figure}
        ...
        \end{subfigure}
        \caption{$W_{\alpha \beta}$ \& $P_{\alpha \beta}$ components of \emph{Configuration 5} using ES model with $\it{b=0.5}$.}
